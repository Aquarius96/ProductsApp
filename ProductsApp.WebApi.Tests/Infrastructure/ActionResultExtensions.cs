...
namespace Microsoft.AspNetCore.Mvc
{
    ...
    public class ActionResultAssertion : ReferenceTypeAssertions<IActionResult, ActionResultAssertion>
    {
        ...
        public ActionResultAssertion BeSuccess<T>(T value,
            string because = "",
            params object[] becauseArgs) where T : class
        {
            Execute.Assertion
                .BecauseOf(because, becauseArgs)
                .ForCondition(Subject.GetType() == typeof(CreatedAtActionResult))
                .FailWith("Subject should be of type Created Result");

            var createdResult = (CreatedAtActionResult) Subject;
            ...
            Execute.Assertion
                .BecauseOf(because, becauseArgs)
                .ForCondition(createdResult.Value == value)
                .FailWith("Values should be the same");

            return this;
        }

        public ActionResultAssertion BeOk<T>(T value,
            string because = "",
            params object[] becauseArgs) where T : class
        {
            Execute.Assertion
